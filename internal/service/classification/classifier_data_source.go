...
func (d *ClassifierDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	// Always a list, never null, so length() and for_each work on a
	// classifier that belongs to no collections.
	names := make([]string, 0, len(classifier.CollectionNames))
	names = append(names, classifier.CollectionNames...)

	collectionNames, diags := types.ListValueFrom(ctx, types.StringType, names)
