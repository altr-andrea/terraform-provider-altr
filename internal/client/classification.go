...
// GetClassifier gets a classifier by name, or nil if it does not exist
...
// GetClassifierCollection gets a collection by name, or nil if it does not exist
...
// ListClassifierCollectionClassifiers lists a collection's member classifiers,
// following the cursor. Returns nil if the collection does not exist
func (c *Client) ListClassifierCollectionClassifiers(collectionName string) ([]Classifier, error) {
	...
	for {
		...
		// Stop if the cursor stops advancing, rather than looping forever.
